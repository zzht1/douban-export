...
### v5 样本扩容与模型重训练 (2026-06-18)

**样本扩容**:
- 种子: 86 → 86（未新增，反爬限制）
- 有特征的用户: 35 → 62 人（+27 人）
- 类型分布: INTJ:9, INFP:8, INTP:7, ENTP:9, INFJ:10, ESTJ:3, ENFJ:2, ISTJ:3, ISFJ:3, ISFP:2, ENFP:2, ENTJ:3, ESTP:1
- S 型: 6 → 9 人（+3 人：ESTP:1, ISTJ:1, ISFJ:1）
- 仍然缺失: ESFJ, ESFP（0 人）

**数据集重建**:
- 有效用户: 50 人（从 35 人提升）
- 数据集: 50 真实用户 → 464 合成样本
  - 分层分割: train=30 真实, val=10 真实, test=10 真实
  - 自适应变体数: 5（因 n=50 在 30-50 区间）

**v5 批量测试** (`test_mbti.py` 对 47 人做全量预测):
...
| FT   | 90.0% | 85.1% | -4.9% |
| JP   | 80.0% | **83.0%** | +3.0% |
| ≥3 维度正确 | 70.0% | **76.6%** | +6.6% |
| 全类型匹配 | 65.0% | 61.7% | -3.3% |

- **关键发现**: 测试集从 20 人扩大到 47 人，准确率变化受测试集组成影响
- **NS 维度提升**: S 型样本从 6 人增加到 9 人，NS 准确率提升 5.1%
- **模型状态**: RandomForest（真实样本 30 < 50 阈值）

### 后续计划
- **首要**: 等待反爬冷却，继续补采至 200 种子
- **NS 维度**: S 型种子 12 人中仅 9 人有数据，需重点补采 ESTP/ESFP/ESFJ
